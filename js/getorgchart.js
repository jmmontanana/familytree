...
getOrgChart = function (b, a) {
  ...
  for (id in this.config.customize) {
    if (this.config.customize[id].theme) {
      this.config.embededDefinitions +=
        getOrgChart.themes[this.config.customize[id].theme].defs;
    }
  }
...
getOrgChart.prototype._f = function () {
  this._zN = get._c().msie
    ? this.element.clientWidth
    : window.getComputedStyle(this.element, null).width;
  this._zN = parseInt(this._zN);
  if (this._zN < 3) {
    this._zN = 1024;
    this.element.style.width = "1024px";
  }
  this._zH = get._c().msie
    ? this.element.clientHeight
    : window.getComputedStyle(this.element, null).height;
...
getOrgChart.prototype._A = function () {
  ...
  e.push(
    getOrgChart.OPEN_SVG.replace(
      "[defs]",
      this.config.embededDefinitions
    ).replace("[viewBox]", g.toString())
  );
...
getOrgChart.prototype._8 = function () {
  ...
  if (this.config.boxSizeInPercentage != null) {
    ...
    this.scale = this._r(
      ...
      this.config.boxSizeInPercentage.boxSize.height,
      c,
      l,
      m
    );
    this.minScale = this._r(
      0,
      this.config.boxSizeInPercentage.minBoxSize.width,
      this.config.boxSizeInPercentage.minBoxSize.height,
      c,
      l,
      m
    );
    this.maxScale = this._r(
      10000000,
      this.config.boxSizeInPercentage.maxBoxSize.width,
      this.config.boxSizeInPercentage.maxBoxSize.height,
      c,
      l,
      m
    );
  ...
  switch (this.config.orientation) {
    ...
    case getOrgChart.RO_TOP_PARENT_LEFT:
      ...
      this.initialViewBoxMatrix = [
        -p + a,
        q + b,
        o + this.config.siblingSeparation,
        d,
      ];
      break;
    case getOrgChart.RO_BOTTOM:
...
getOrgChart.prototype._z = function (a) {
  ...
  for (var d in this.nodes) {
    ...
    if (
      this.nodes[d]._zJ == this.nodes[d].x &&
      this.nodes[d]._zM == this.nodes[d].y
    ) {
      continue;
    }
...
getOrgChart._X.prototype._zG = function (a) {
  ...
  for (i = 0; i < c.length; i++) {
    ...
    while (b > d) {
      c[i].textContent = c[i].textContent.substring(
        0,
...
getOrgChart.OPEN_SVG =
  '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="[viewBox]"><defs>[defs]</defs><g>';
getOrgChart.CLOSE_SVG = "</svg>";
getOrgChart.OPEN_NODE =
  '<g data-node-id="[data-node-id]" class="get-level-[level] [nodeCssClass]" transform="matrix(1,0,0,1,[x],[y])">';
...
getOrgChart._T = function (h, g, d) {
  ...
  if (g.collapsed == getOrgChart.NOT_DEFINED) {
    g.collapsed =
      h.config.expandToLevel && h.config.expandToLevel <= d && g._N()
        ? getOrgChart.COLLAPSED
        : getOrgChart.EXPANDED;
  }
...
getOrgChart.node.prototype._Z = function (a) {
  ...
  var c = null;
  var e =
    a.customize[this.id] && a.customize[this.id].theme
      ? getOrgChart.themes[a.customize[this.id].theme]
    ...
    if (a.expandToLevel && this.displayChildren == null) {
      d.push(
        getOrgChart.buttons.expColl
          ...
          .replace(/\[xa]/g, p - e.expandCollapseBtnRadius)
          .replace(/\[ya]/g, u - e.expandCollapseBtnRadius)
          .replace(/\[start]/g, (e.expandCollapseBtnRadius * 2) / 6)
          .replace(/\[middle]/g, e.expandCollapseBtnRadius)
          .replace(/\[end]/g, ((e.expandCollapseBtnRadius * 2) / 6) * 5)
...
getOrgChart.node.prototype._W = function (c) {
  ...
  var e = null;
  var g =
    c.customize[this.id] && c.customize[this.id].theme
      ? getOrgChart.themes[c.customize[this.id].theme]
...
getOrgChart.node.prototype.draw = function (a) {
  ...
  var f =
    a.customize[this.id] && a.customize[this.id].theme
      ? " get-" + a.customize[this.id].theme
      : "";
  var e =
    a.customize[this.id] && a.customize[this.id].color
      ? " get-" + a.customize[this.id].color
      : "";
  if (f && !e) {
    e = " get-" + a.color;
  }
  ...
  h.push(
    getOrgChart.OPEN_NODE.replace("[data-node-id]", this.id)
      .replace("[x]", this._zJ == null ? this.x : this._zJ)
      .replace("[y]", this._zM == null ? this.y : this._zM)
...
getOrgChart.themes = {
  ...
  belinda: {
    ...
    text: '<text width="[width]" class="get-text get-text-[index]" x="[x]" y="[y]">[text]</text>',
    image:
      '<clipPath id="getBelindaClip1"><rect x="0" y="75" width="300" height="150" /></clipPath><clipPath clip-path="url(#getBelindaClip1)" id="getBelindaClip2"><circle cx="150" cy="150" r="150" /></clipPath><image preserveAspectRatio="xMidYMid slice"  clip-path="url(#getBelindaClip2)" xlink:href="[href]" x="1" y="1" height="300"   width="300"/>',
  },
  cassandra: {
    size: [310, 140],
    toolbarHeight: 46,
  ...
  deborah: {
    ...
    expandCollapseBtnRadius: 20,
    image:
      '<clipPath id="getVivaClip"><path class="get-box" d="M35 0 L187 0 Q222 0 222 35 L222 187 Q222 222 187 222 L35 222 Q0 222 0 187 L0 35 Q0 0 35 0 Z"/></clipPath><image clip-path="url(#getVivaClip)" xlink:href="[href]" x="0" y="0" height="222" preserveAspectRatio="xMidYMid slice" width="222"/>',
  ...
  monica: {
    ...
    text: '<text width="[width]" class="get-text get-text-[index]" x="[x]" y="[y]">[text]</text>',
    image:
      '<clipPath id="getMonicaClip"><circle cx="105" cy="65" r="85" /></clipPath><image preserveAspectRatio="xMidYMid slice" clip-path="url(#getMonicaClip)" xlink:href="[href]" x="20" y="-20" height="170" width="170"/>',
  },
  ula: {
    size: [500, 220],
    toolbarHeight: 46,
  ...
  vivian: {
    ...
    text: '<text width="[width]" class="get-text get-text-[index]" x="[x]" y="[y]">[text]</text>',
    image:
      '<clipPath id="getVivianClip"><polygon class="get-box" points="20,70 75,-20 185,-20 240,70 185,160 75,160"/></clipPath><image preserveAspectRatio="xMidYMid slice" clip-path="url(#getVivianClip)" xlink:href="[href]" x="20" y="-20" height="200" width="300"/>',
  },
  ada: {
    size: [500, 220],
    toolbarHeight: 46,
  ...
  if (a < 1 / 2.75) {
    ...
    if (a < 2 / 2.75) {
      ...
      if (a < 2.5 / 2.75) {
        ...
        return (
          1 - (7.5625 * (a - 2.625 / 2.75) * (a - 2.625 / 2.75) + 0.984375)
        );
      }
    }
  }
...
get._z._ad = function (a) {
  ...
  return a < 0.5
    ? 0.5 * (4 * a * a * ((2.5949 + 1) * 2 * a - 2.5949))
    : 0.5 *
        ((2 * a - 2) * (2 * a - 2) * ((2.5949 + 1) * (2 * a - 2) + 2.5949) + 2);
};
...
get._c = function () {
  ...
  var b =
    ...
    e.exec(g) ||
    d.exec(g) ||
    (g.indexOf("compatible") < 0 && c.exec(g)) ||
    [];
...
getOrgChart.util._6 = function (f) {
  ...
  for (var e = 0; e < d.length; e++) {
    ...
    if (c && c.length == 2 && c[0] === f) {
      ...
      var g = /(%[^%]{2})/;
      while (
        (encodedChar = g.exec(c[1])) != null &&
        encodedChar.length > 1 &&
        encodedChar[1] != ""
      ) {
...
getOrgChart.prototype.showDetailsView = function (d) {
  ...
  var b = function (p, n, q) {
    ...
    var r =
      "<select " +
      l +
      'class="get-oc-select-parent"><option value="' +
      p +
    ...
    for (var k in n) {
      ...
      if (o.id == p) {
        r +=
          '<option selected="selected" value="' + o.id + '">' + s + "</option>";
      } else {
  ...
  for (label in h.data) {
    c += getOrgChart.DETAILS_VIEW_INPUT_HTML.replace(
      /\[label]/g,
      label
  ...
  this._k();
  var j =
    this.config.customize[h.id] && this.config.customize[h.id].theme
      ? getOrgChart.themes[this.config.customize[h.id].theme].toolbarHeight
...
getOrgChart.prototype._j = function (l, a) {
  ...
  var e = getOrgChart.DETAILS_VIEW_INPUT_HTML.replace(/\[label]/g, h).replace(
    "[value]",
    ""
  );
...
getOrgChart.prototype.showMainView = function () {
  ...
  this._X._9.innerHTML = "";
  if (
    this._X._zF.style.top != 0 &&
    this._X._zF.style.top != "" &&
    this._X._zF.style.top != "0px"
  ) {
...
getOrgChart.prototype.highlightNode = function (c) {
  ...
  function b() {
    ...
    a.move(
      [f, g, a.initialViewBoxMatrix[2], a.initialViewBoxMatrix[3]],
      null,
      function () {
...
getOrgChart.prototype._l = function () {
  if (
    this._zj.currentIndex < this._zj.found.length - 1 &&
    this._zj.found.length != 0
  ) {
...
getOrgChart.prototype._aB = function (g, a) {
  ...
  if (
    this.config.enableEdit == true ||
    getOrgChart.util._s(this.config.enableEdit, c)
  ) {
    this._zp("add", f, b, c);
    this._zp("edit", d, h, c);
    this._zp("del", d, b, c);
...
getOrgChart.prototype._h = function (d, f, g, c, a, b) {
  var h =
    this.config.customize[d] && this.config.customize[d].theme
      ? getOrgChart.themes[this.config.customize[d].theme]
      : this.theme;
  a = a == undefined ? getOrgChart.NOT_DEFINED : a;
  var e = new getOrgChart.node(
    d,
    f,
    g,
    c,
    h.size,
    this.config.photoFields,
    a,
    b
  );
...
getOrgChart.prototype.exportToImage = function () {
  ...
  var s =
    ...
    m +
    "," +
    (j + this.theme.size[0] + this.config.subtreeSeparation * 2) +
    "," +
    (k + this.theme.size[1]);
    ...
    get._a._B(f, null, function (t) {
      ...
      u = u
        ...
        .replace(
          ...
          '<defs/><g class="' +
            o._X._zY.className +
            '"><g class="get-oc-tb"><g class="get-oc-c">'
        )
        .replace(
          "</defs>",
          '</defs><g class="' +
        ...
        .replace(
          "<defs/>",
          '<defs><style type="text/css">' + t + "</style></defs>"
        )
        .replace("</defs>", '<style type="text/css">' + t + "</style></defs>")
        .replace("</svg>", "</g></g></g></svg>");
